...
namespace CodeFactory.ADK
{
    ...
    public interface IInjectInstanceContainerMemberSyntax<TContainerType, TMemberType>  where TContainerType: CsContainer where TMemberType : CsMember
    {
        /// <summary>
        ...
        Task<IUpdateInstanceSource<TContainerType>> InjectSyntaxAsync(IUpdateInstanceSource<TContainerType> updateSource,
            TMemberType member,int indentLevel,LogLevel defaultLogLevel = LogLevel.Information, CsSecurity targetSecurity = CsSecurity.Unknown, bool replace = false);
    }
}
